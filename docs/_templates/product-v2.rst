...
.. tab-item:: Specifications
   ...
   Vector products contain one or more distinct layers of data, and each layer can contain multiple attribute fields. The product {{ product_ids_list_text }} contains the following layers: {% for layer in layers_table_list %}`{{ layer.name }} <./#layer-{{ loop.index }}>`_{% if not loop.last %}, {% endif %}{% endfor %}.
